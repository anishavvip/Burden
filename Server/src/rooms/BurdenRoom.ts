...
export class BurdenRoom extends Room<ColyseusRoomState> {
  ...
  onEntityUpdate(clientID: string, data: any) {
    ...
    for (let i = startIndex; i < data.length; i += 2) {
      ...
      if (updateValue === "inc") {
        ...
        updateValue =
          parseFloat(stateToUpdate.attributes.get(property)) +
  ...
  async onCreate(options: any) {
    ...
    this.onMessage("remoteFunctionCall", (client, RFCMessage) => {
      ...
      this.broadcast(
        "onRFC",
        RFCMessage,
        RFCMessage.target == 0 ? {} : { except: client }
      );
    });
    ...
    this.onMessage("createEntity", (client, creationMessage) => {
      ...
      for (let key in creationMessage.attributes) {
        if (key === "creationPos") {
          ...
          newEntity.attributes.set(
            key,
            creationMessage.attributes[key].toString()
          );
        }
      }
    ...
    this.setSimulationInterval((dt) => {
      ...
      try {
        if (customLogic != null) customLogic.ProcessLogic(this, dt);
      } catch (error) {
        logger.error("Error with custom room logic: " + error);
      }
    });
  ...
  setAttribute(client: Client, attributeUpdateMessage: any) {
    if (
      ...
      (attributeUpdateMessage.entityId == null &&
        attributeUpdateMessage.userId == null) ||
    ...
    if (attributeUpdateMessage.entityId) {
      ...
      if (
        this.state.networkedEntities.has(
          `${attributeUpdateMessage.entityId}`
        ) === false
      )
        return;

      this.state.networkedEntities.get(
        `${attributeUpdateMessage.entityId}`
      ).timestamp = parseFloat(this.serverTime.toString());
      let entityAttributes = this.state.networkedEntities.get(
        `${attributeUpdateMessage.entityId}`
      ).attributes;
      for (
        let index = 0;
        index < Object.keys(attributeUpdateMessage.attributesToSet).length;
        index++
      ) {
    ...
    // Set user attribute
      ...
      if (
        this.state.networkedUsers.has(`${attributeUpdateMessage.userId}`) ===
        false
      ) {
        logger.error(
          `Set Attribute - User Attribute - Room does not have networked user with Id - \"${attributeUpdateMessage.userId}\"`
        );
        return;
      }

      this.state.networkedUsers.get(
        `${attributeUpdateMessage.userId}`
      ).timestamp = parseFloat(this.serverTime.toString());

      let userAttributes = this.state.networkedUsers.get(
        `${attributeUpdateMessage.userId}`
      ).attributes;
  ...
  async onLeave(client: Client, consented: boolean) {
    ...
    try {
      ...
      logger.info(
        "let's wait for reconnection for client: " + client.sessionId
      );
      ...
      if (this.clientEntities.has(client.id)) {
        ...
        logger.error(
          `Can't remove entities for ${client.id} - No entry in Client Entities!`
        );
      }
    }
  }
