...
public class PlayerController : ExampleNetworkedEntityView
{
    ...
    private void GetSyncData(InputSyncData input)
    {
        ...
        if (input.name == prefabName)
        {
            ...
            if (_hasAnimator)
            {
                ...
                _animationBlend = Mathf.Lerp(_animationBlend, move.magnitude * playerSpeed, Time.deltaTime * SpeedChangeRate);
    ...
    protected override void Update()
    {
        ...
        if (!hasGameBegun)
            return;
        TakeInputs();
        HandleInput(SyncData.left, SyncData.right, SyncData.up, SyncData.down, SyncData.jump);
    ...
    private void HandleInput(bool left, bool right, bool up, bool down, bool jump)
    {
        if (isPaused) return;
